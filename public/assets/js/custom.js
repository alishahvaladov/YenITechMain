...
$.get('/api/profile/profile-picture', (res) => {
   ...
   if (res.filename !== null) {
      ...
      avatarPicture.classList.remove("d-none");
      avatars.forEach(item => {
         item.src = res.filename;
      });
   } else {
      const letterAvatar = document.querySelector(".letter-avatar");
      const letterNameHeader = document.querySelector(".avatar-letters");
      const full_name = res.full_name;
      const splittedName = full_name.split(" ");
      const letterName = `${splittedName[0][0]}${splittedName[1][0]}`
      letterNameHeader.innerHTML = letterName;
      letterAvatar.classList.remove("d-none");
      letterAvatar.classList.add("d-flex");
   }
