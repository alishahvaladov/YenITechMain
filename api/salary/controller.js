const {
  createSalaryRecordAndSendEmail,
  getFines,
  getSalary,
  getSalaryByMonthByEmpID,
  getTimeOffs,
  getEmployeeExperience,
  getSalariesByMonth,
  getMonthlyWorkingDays,
  addCalculatedGrossToDB,
  search,
  searchSalaryByMonts,
  getSalariesForExport,
  getSalaryByID,
  updateSalary,
  getSalariesForExportAll,
  getCalculatedSalary,
} = require("./service");
...
function timeOffCalculation(timeOffs, monthLySalaries, emp_id, gross, montlyWorkingDays) {
  ...
  if (date === 1) {
    ...
    daysOfWork =
      parseInt(lastDayOfMonth) - parseInt(weekendsBySunAndSat.sunCount) - parseInt(weekendsBySunAndSat.satCount);
  } else if (date > 27) {
